...
func TestSummaryEndpoint(t *testing.T) {
	...
	defer srv.Close()
	resp, _ := authGet(srv.URL + "/api/v1/transactions/abc/summary")
...
func TestServicesAndREDEndpoints(t *testing.T) {
	srv := httptest.NewServer(Router(fakeDerived{}))
	defer srv.Close()
	r1, _ := authGet(srv.URL + "/api/v1/services")
	var svcs []string
	json.NewDecoder(r1.Body).Decode(&svcs)
	if len(svcs) != 2 || svcs[0] != "SvcA" {
		t.Fatalf("services %v", svcs)
	}
	r2, _ := authGet(srv.URL + "/api/v1/services/SvcA/red?from=2020-01-01T00:00:00Z&to=2030-01-01T00:00:00Z")
	var pts []REDPointDTO
